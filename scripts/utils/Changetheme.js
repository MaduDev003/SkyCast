...
function applyUITheme(theme) {
    ...
    el.legends.forEach(item => item.style.color = styles.legend);
    el.icons.forEach(item => item.style.color = styles.legend);
    ...
    if (el.dayWeather) {
        ...
        el.weatherDescription.forEach(p => {
            p.style.color = styles.legend;
        });

        el.weatherTemperature.forEach(h3 => {
            h3.style.color = styles.main;
        });
    }

    el.forecastItems.forEach(li => {
        li.style.color = styles.legend;
    });
